master:
    Finalny
a_r_1:
	Kopia bezpieczeństwa mastera
a_r_2:
    Roboczy - program testuje w pętli model (1 epochs - do zmiany)


air_compressor_3_keras.py:
                    podstawowy, liczy sieć na oryginalnych próbkach (które nie są różnowartościowe)
air_compressor_keras_dane_roznowartosc.py:
                    Próbki różnowartościowe
                    ##Program eliminuje w danych próbki powtarzające się
                    ##Liczba próbek została obniżona o ponad połowę
<<<<<<< HEAD
                    ##Nie widać praktycznie różnicy w dokładności w porównaniu do air_compressor_3_keras.py
=======
                    ##Nie widać praktycznie różnicy w dokładności

     MODELE - pliki
>>>>>>> a_r_1
air_compressor_model.h5:
                    model sieci: 8 wejść, 3 wyjścia; [15,15,3]
air_compressor_model_25_25_3_5e_40_60_2010.06.21.h5:
                    model sieci: [25,25,3], 5 epochs, test_size=0.4

